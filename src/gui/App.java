...
public class App {
	...
	public static void main(String[] args) {
		 ...
		 * "ProjetoFinalJavaWebPersistencia"); EntityManager em =
		 * emf.createEntityManager(); BorrachariaService service = new
		 * BorrachariaService(em); MotoristaService servicemotora = new
		 * MotoristaService(em);
		 **/
		...
		/**
		 * em.getTransaction().begin(); Borracharia borracharia =
		 * service.createEmployee(1, "ASDF", "ASDF", "ASDF", "ASDF");
		 * em.getTransaction().commit(); System.out.println("Persisted " +
		 * borracharia);
		 **/

		// create motora
		/**
		 * em.getTransaction().begin(); motoraMotorista =
		 * servicemotora.CreateMotorista(1, "tadeu", "1231232", "123213",
		 * "43423", "1241412", "2014", "gasolina");
		 * em.getTransaction().commit(); System.out.println("persistido" +
		 * motoraMotorista);
		 **/
		...
		for (DocumentoAnual d : daoDocAnu.getAll(DocumentoAnual.class)) {
		...
		/**
		 * em.getTransaction().begin(); borra.setCodigo(1);
		 * borra.setContato("fsjldfjsd"); borra.setDescricao("fasdasd");
		 * borra.setLocal("2323"); borra.setTelefone("12"); em.merge(borra);
		...
		/**
		 * List<Borracharia> borra = service.findAllBorracharia(); for
		 * (Borracharia e : borra) System.out.println("Found Borracharia: " +
		 * e.getDescricao());
		 */

		// find a specific borracharia
		/**
		 * borra = service.findBorracharia(1); System.out.println("Found " +
		 * borra.getLocal());
		 */

		// remove an employee
		/**
		 * em.getTransaction().begin(); service.removeBorracharia(1);
		 * em.getTransaction().commit();
